...
/**
 * @author : Kunal Sharma
 * @since : 21/07/24, Sunday
 **/
//T.C : O(k*^2 + n)
//S.C : O(k*^2 + n)
public class BuildWithMatrix {
    ...
    public int[][] buildMatrix(int k, int[][] rowConditions, int[][] colConditions) {
        ...
        for (int i = 0; i < k; i++) {
            positionMap.put(orderColumns.get(i), i); //  colVal -> index
        }

        for (int i = 0; i < k; i++) {
            int element = orderRows.get(i); //   rowVal
    ...
    //T.C : O(k*^2 + n)
//S.C : O(k*^2 + n)
    public int[][] buildMatrixUsingBFS(int k, int[][] rowConditions, int[][] colConditions) {
        List<Integer> orderRows = topoSortUsingBFS(rowConditions, k); // O(K)
        List<Integer> orderColumns = topoSortUsingBFS(colConditions, k);
    ...
    private List<Integer> topoSortUsingBFS(int[][] edges, int n) {
